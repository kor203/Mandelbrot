...
public class FractalGenerator002 {
    ...
    public static class poolGenerator extends Thread{
        BufferedImage img;
        positionOfPixel[] pos;
        int max_iterations;

        public poolGenerator(positionOfPixel[] pos, int max_iterations, BufferedImage img){
            this.img = img;
            this.pos = pos;
    ...
    public static BufferedImage generator(int xpx, int ypx, double minx, double maxx, double miny, double maxy, int max_iterations) throws IOException {
        ...
        poolGenerator[] generators = new poolGenerator[splits.length];
        for (int i = 0; i < splits.length; i++) {
            generators[i] = new poolGenerator(splits[i], max_iterations, img);
            generators[i].start();
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        BufferedWriter output_file = new BufferedWriter(new FileWriter("times_002.txt"));
        ...
        output_file.close();*/
        drawFractal(generator(1024, 1024, -2.1, 0.6, -1.2, 1.2, 200));
    }
}